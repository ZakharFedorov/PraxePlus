 ...
 */
public class FXMLAdminEditovatPraxeController implements Initializable {
    ...
    @FXML
    private Button buttonUlozitPraxe;
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        temaNovaPraxe.setText(practice.getTopic());
        ...
        buttonUlozitPraxe.setOnAction(event ->{
            
            try {
                daow.deleteP(keysList);
            } catch (SQLException ex) {
                Logger.getLogger(FXMLAdminEditovatPraxeController.class.getName()).log(Level.SEVERE, null, ex);
            }
            ...
            if((emailKONovaPraxe.getText().length() > 0)&&(jmenoKONovaPraxe.getText().length() > 0)
                ...
                if((ValidMail.validate(emailKONovaPraxe.getText()) == true)){
                    ...
                    s.setYear(practice.getYear());
                    s.setStudent_id(practice.getStudent_id());
                    s.setTeacher_id(practice.getTeacher_id());

                    if((s.getStudent_id() == 0) && (s.getTeacher_id() == 0)){
                        try {
                            dao.saveNewP(s);
                        } catch (SQLException ex) {
                            Logger.getLogger(FXMLAdminEditovatPraxeController.class.getName()).log(Level.SEVERE, null, ex);
                    ...
                    buttonUlozitPraxe.getScene().getWindow().hide();
                }else{
                    emailKONovaPraxe.setStyle("-fx-border-color:red");
                }
            }
